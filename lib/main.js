...
import { bindAll, debounce, extend } from 'lodash';
...
class Autocomplete extends React.Component {
  ...
  constructor(props) {
    ...
    const debounceValue = this.props.debounce === 0 ? 0 : this.props.debounce || 500;

    bindAll(this, [ 'hideItems', 'onSelectIndex', 'onKeyDown', 'onMouseOver', 'onClickItem' ])
    this.onChangeInput = debounce(function() {
      this.setState({
        loading: true
      });
      this.props.onChange(ReactDOM.findDOMNode(this.refs.input).value);
    }.bind(this), debounceValue, { leading: false });
